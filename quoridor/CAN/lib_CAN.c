...
void CAN_setup (uint32_t ctrl)  {
  ...
  if (ctrl == 1) {
    LPC_SC->PCONP       |=  (1 << 13);           /* Enable power to CAN1 block slide 64 */
    LPC_PINCON->PINSEL0 |=  (1 <<  0);           /* Pin P0.0 used as RD1 (CAN1) slide 117*/
  ...
  LPC_CANAF->AFMR = 2;                           /* By default filter is not used  slide 386 */
  pCAN->MOD   = 1;                               /* Enter reset mode slide 361 */
  pCAN->IER   = 0;                               /* Disable all interrupts  slide 369 */
...
/*----------------------------------------------------------------------------
  write a message to CAN peripheral and transmit it.  CAN controller (1..2)
 *----------------------------------------------------------------------------*/
